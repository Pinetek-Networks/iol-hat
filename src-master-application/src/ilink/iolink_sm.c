...
{
	 LOG_DEBUG (IOLINK_DL_LOG,"sm_DL_SetMode_req_startup_inactive>>>\n");
   iolink_mode_vl_t valuelist;
   ...
   iolink_dl_mode_t mode)
{
	...
	LOG_DEBUG (IOLINK_DL_LOG,"sm_DL_SetMode_req_preop_op>>>\n");
   iolink_mode_vl_t valuelist;
   iolink_sm_port_t * sm                       = iolink_get_sm_ctx (port);
   iolink_smp_parameterlist_t * real_paramlist = &sm->real_paramlist;
...
{
	LOG_DEBUG (IOLINK_DL_LOG,"sm_DL_SetMode_req_preop>>>\n");
...
{
	LOG_DEBUG (IOLINK_DL_LOG,"sm_DL_SetMode_req_operate>>>\n");
   ...
   iolink_fsm_sm_event_t event)
{
	...
	LOG_DEBUG (IOLINK_DL_LOG,"sm_inspectFault>>>\n");
   switch (event)
   {
...
static inline void sm_DL_Write_cnf_waiton_operate (iolink_job_t * job)
{
   ...
   LOG_DEBUG (IOLINK_SM_LOG, "sm_DL_Write_cnf_waiton_operate\n");
...
{
		LOG_DEBUG(IOLINK_SM_LOG, "sm_DL_Mode_ind_cb_preop >>>\n");
...
{
	   LOG_DEBUG (
      IOLINK_SM_LOG, "sm_DL_Mode_ind_cb>>>\n");
   iolink_port_t * port     = job->port;
   iolink_mhmode_t realmode = job->dl_mode;
   iolink_sm_port_t * sm    = iolink_get_sm_ctx (port);
...
static void sm_DL_Write_cnf_cb (iolink_job_t * job)
{
   iolink_port_t * port = job->port;
	 ...
	 LOG_DEBUG (IOLINK_SM_LOG,"sm_DL_Write_cnf_cb>>>\n");
...
static void sm_DL_Write_Devmode_cnf (iolink_job_t * job)
{
   ...
   iolink_sm_port_t * sm = iolink_get_sm_ctx (port);
	 ...
	 LOG_DEBUG (IOLINK_SM_LOG, "sm_DL_Write_Devmode_cnf\n");
...
void DL_Write_cnf (iolink_port_t * port, iolink_status_t errorinfo)
{
   iolink_sm_port_t * sm = iolink_get_sm_ctx (port);
	 ...
	 LOG_DEBUG (IOLINK_SM_LOG,"DL_Write_cnf>>>\n");
   ...
   iolink_post_job_with_type_and_callback (
      ...
      sm_DL_Write_cnf_cb);
		...
		LOG_DEBUG (IOLINK_SM_LOG,"DL_Write_cnf: job posted\n");
}
...
{
	LOG_DEBUG (IOLINK_SM_LOG, "DL_Mode_ind>>>\n");
   iolink_job_t * job = iolink_fetch_avail_job (port);
   ...
   iolink_post_job_with_type_and_callback (
      ...
      sm_DL_Mode_ind_cb);
		LOG_DEBUG (IOLINK_SM_LOG, "DL_Mode_ind: job posted..\n");				
}
