   ...
   uint32_t masterid)
{
       ...
       IOLINK_SMI_ERRORTYPE_NONE))
   {
		 ...
		 LOG_DEBUG(LOG_STATE_ON, "%s: error=%d, return 1\n", __func__,err);
      return 1;
   }
	 
	 
	 LOG_DEBUG(LOG_STATE_ON, "vendorid=%d\n", app_port->app_master->vendorid);
	 LOG_DEBUG(LOG_STATE_ON, "masterid=%d\n", app_port->app_master->masterid);
...
static uint8_t iolink_config_port_sdci_auto (iolink_app_port_ctx_t * app_port)
{
       ...
       IOLINK_SMI_ERRORTYPE_NONE))
   {
		 ...
		 LOG_DEBUG (LOG_STATE_ON, "%s: err=%d\n", __func__, err);
      return 1;
   }
   ...
   iolink_pl_mode_t port_mode)
{
   ...
   case iolink_mode_INACTIVE:
	...
	LOG_DEBUG (LOG_STATE_ON, "%s: result = %d\n", __func__, res);
   return res;
}
...
static uint8_t iolink_start_port (iolink_app_port_ctx_t * app_port)
{
	...
	LOG_DEBUG (LOG_STATE_ON, "=============================================>>>>  iolink_start_port\n");
   ...
   if (port_status->port_status_info != IOLINK_PORT_STATUS_INFO_OP)
   {
	...
	if (app_port->portnumber == 1)
		generic_setup0(app_port);
