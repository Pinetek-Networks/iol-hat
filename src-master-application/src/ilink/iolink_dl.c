...
static inline bool get_isdu_total_len (iolink_dl_t * dl, uint8_t * isdu_total_len)
{
   uint8_t len = get_isdu_len (dl);
	 ...
	 //LOG_DEBUG (LOG_STATE_ON, "get_isdu_total_len, iservice =%d, len=%d\n", (dl->isdu_handler.isdu_data[0] & 0xF0) >> 4,len);

   if (len == 1)
   {
      len = dl->isdu_handler.isdu_data[1];
			//LOG_DEBUG (LOG_STATE_ON, "len=%d\n", len);
   }
...
static void iolink_dl_isdu_h_sm_reception_complete (iolink_port_t * port)
{
   ...
   {
		 LOG_DEBUG (LOG_STATE_ON, "iolink_dl_isdu_h_sm_reception_complete, !get_isdu_data_length (%d)\n", isdu_data_length);
...
static inline void iolink_dl_handle_error (iolink_port_t * port)
{
   ...
   {
		  LOG_ERROR (IOLINK_DL_LOG,"%s dl->rxtimeout = true\n", __func__);
      dl->rxtimeout = true;
      iolink_dl_message_h_sm (port);
   }
   else if ((dl->cqerr & (BIT (3) | BIT (1) | BIT (0))) != 0) // RChksmEr/FrameErr/ParityErr
   {
	...
	{
		LOG_DEBUG(IOLINK_DL_LOG, "iolink_dl_handle_error: error not handled\n");
	}
#endif
}
         ...
         if (dl->triggered_events & IOLINK_DL_EVENT_TIMEOUT)
         {
            ...
            case IOL_DL_TIMER_TINITCYC_MH:
               stop = clock();
							 LOG_DEBUG (
                  IOLINK_DL_LOG,
                  "%s: TInitcyc timed out. IOL_DL_MH state: %s, %.3f\n",
                  __func__,
                  iolink_dl_mh_st_literals[dl->message_handler.state], (double)(stop - start) / CLOCKS_PER_SEC);
